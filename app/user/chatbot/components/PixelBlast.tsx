...
const PixelBlast: React.FC<PixelBlastProps> = ({
  ...
  useEffect(() => {
    ...
    if (mustReinit) {
      ...
      const canvas = document.createElement('canvas');
      // Try WebGL2 first, fallback to WebGL1 for mobile compatibility
      let gl = canvas.getContext('webgl2', { antialias, alpha: true, preserveDrawingBuffer: false, powerPreference: 'high-performance' });
      if (!gl) {
        gl = canvas.getContext('webgl', { antialias, alpha: true, preserveDrawingBuffer: false, powerPreference: 'high-performance' }) as WebGL2RenderingContext;
      }
      if (!gl) {
        console.warn('WebGL not supported, PixelBlast will not render');
        return;
      }
      ...
      // Force initial sizing after DOM insertion
      const forceResize = () => {
        const containerRect = container.getBoundingClientRect();
        const w = Math.max(containerRect.width || container.clientWidth || container.offsetWidth, 1);
        const h = Math.max(containerRect.height || container.clientHeight || container.offsetHeight, 1);
      ...
      const uniforms = {
        ...
        uScale: { value: isMobile ? patternScale * 0.8 : patternScale },
        uDensity: { value: isMobile ? patternDensity * 0.7 : patternDensity },
        uPixelJitter: { value: isMobile ? pixelSizeJitter * 0.5 : pixelSizeJitter },
      ...
      // Add Intersection Observer for mobile optimization
